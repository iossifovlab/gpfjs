      ...
      this.route.params.pipe(
        ...
        next: dataset => {
          if (dataset) {
            ...
            this.store1.select(selectDatasetId).pipe(take(1)).subscribe((datasetId: string) => {
              setDatasetId({datasetId: dataset.id}); // must join on more conditions most likely
            });
            this.store1.subscribe(state => console.log(state));
